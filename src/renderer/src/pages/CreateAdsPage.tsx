    ...
    <main className="flex flex-1 justify-center overflow-y-auto">
      <div className="flex min-h-full w-full max-w-5xl items-center px-6 py-8 md:px-10">
        <div className="flex w-full flex-col gap-8">
          ...
          {/* Step body — keyed on `step` so the subtree remounts and plays
              the entry animation each time the user navigates. Height is
              fixed so the progress indicator, title, and footer buttons stay
              in the same position across every step. */}
          {/* Step body — the ad style step and the results grid both use
              the full container width so all thumbnails / all 4 generations
              fit on a single row. Every other step stays constrained to the
              narrower wizard width.

              Height is fixed at 360px for the input steps so the footer
              buttons sit in the same place across them. The results step
              grows naturally so tall 9:16 portrait generations aren't
              clipped — the outer <main> handles overall page scroll. */}
          <div
            key={step}
            className={`animate-step-in hide-scrollbar ${
              step === 'results' ? '' : 'h-[360px] overflow-y-auto'
